...
function buildPrBody(report, plan) {
  const safe = plan.safe.map((c) => `- ${c.path}`).join("\n") || "- None";
  const review = plan.review.map((c) => `- ${c.path}`).join("\n") || "- None";
  return `## RepoReady Fix PR\n\nThis PR makes the repository easier for Codex, Claude Code, Cursor, and contributors to understand.\n\n## Scores\n\n- Overall: ${report.scores.overall}/100\n- Agent Ready: ${report.scores.agentReady}/100\n- Contributor Ready: ${report.scores.contributorReady}/100\n- Context Quality: ${report.scores.contextQuality}/100\n- Safety: ${report.scores.safety}/100\n\n## Safe automatic fixes\n${safe}\n\n## Review-required fixes\n${review}\n\n## Safety\nRepoReady does not run repository scripts and does not merge this PR automatically. Please review before merging.\n`;
}
