...
export async function createFixPullRequest({ repoInput, installationId }) {
  ...
  const prBody = buildPrBody(report, plan);
  const validationCommands = getValidationCommands(report);
  const changeDetails = changes.map(toChangeDetail);

  if (!changes.length) {
    return { ok: true, skipped: true, message: "No fixes required.", report, diff, copyablePatch: diff, prBody, validationCommands, changeDetails };
  }
  ...
  return { ok: true, pullRequestUrl: pr.html_url, branch, changes: changes.map((c) => c.path), changeDetails, report, diff, copyablePatch: diff, prBody, validationCommands };
}
...
function getValidationCommands(report) {
  return [
    report.commands?.test || null,
    report.commands?.build || null,
    report.commands?.lint || null,
    report.commands?.typecheck || null
  ].filter(Boolean);
}

function toChangeDetail(change) {
  return {
    path: change.path,
    action: change.action || "create_or_update",
    risk: change.risk || "review",
    reason: change.reason || "RepoReady generated this reviewable change."
  };
}
